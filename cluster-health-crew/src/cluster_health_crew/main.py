"""Main entry point for the Cluster Health Crew agent."""
...
from cluster_health_crew.crew import ClusterHealthCrew
...
def main():
    ...
    app = KAgentApp(crew=ClusterHealthCrew().crew(), agent_card=agent_card)

    # 3. Build the FastAPI app and run the server
    server = app.build()
    port = int(os.getenv("PORT", "8080"))
    host = os.getenv("HOST", "0.0.0.0")
    logger.info(f"Starting Cluster Health Crew server on {host}:{port}")
    uvicorn.run(
        server,
        host=host,
        port=port,
